...
const submit = (): void => {
    ...
    const q = '/api/auth/login'

    api(q, { method: 'post', body: form.value })
        .then(async () => {
            await fetchUser()

            const redirect = route.query.redirect
            const target = typeof redirect === 'string' && redirect ? redirect : { name: 'admin' }
    ...
    <div class="bg-surface-50 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden">
        <div class="flex flex-col items-center justify-center">
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 py-10 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 text-3xl font-medium mb-4">Welcome to LMS!</div>
                        <span class="text-muted-color font-medium">Sign in to continue</span>
                    </div>

                    <Fluid class="flex flex-col gap-4">
                        <div>
                            <label for="login" class="block font-bold mb-3 required">Email or phone</label>
                            <InputText id="login" v-model="form.login" :invalid="!!errors?.login?.length" />
                            <Message v-if="errors?.login?.length" severity="error" variant="simple">{{ errors?.login }}</Message>
                        </div>

                        <div>
                            <label for="password" class="block font-bold mb-3 required">Password</label>
                            <Password id="password" v-model="form.password" :toggle-mask="true" :feedback="false" />
                        </div>

                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center">
                                <Checkbox v-model="form.remember" input-id="remember" binary class="mr-2" />
